...
OPGP_ERROR_STATUS mutual_authentication(OPGP_CARD_CONTEXT cardContext, OPGP_CARD_INFO cardInfo, BYTE baseKey[16],
	...
	sendBuffer[i] = 0x00;
#ifdef DEBUG
	OPGP_log_Hex(_T("mutual_authentication: sendBuffer: "), sendBuffer, sendBufferLength);
#endif

	status = OPGP_send_APDU(cardContext, cardInfo, NULL, sendBuffer, sendBufferLength, recvBuffer, &recvBufferLength);
#ifdef DEBUG
	OPGP_log_Msg(_T("mutual_authentication: status.errorStatus: 0x%02X"), status.errorStatus);
	OPGP_log_Msg(_T("mutual_authentication: status.errorCode: 0x%02X"), status.errorCode);
	OPGP_log_Msg(_T("mutual_authentication: status.errorMessage: %s"), status.errorMessage);
#endif
	//status.errorStatus = OPGP_ERROR_STATUS_SUCCESS; 
	//status.errorStatus = 0x9000; 
	//memcpy(status.errorMessage,"Success",7);
	if ( OPGP_ERROR_CHECK(status)) {
		goto end;
	}
	CHECK_SW_9000(recvBuffer, recvBufferLength, status);
#ifdef DEBUG
	OPGP_log_Hex(_T("mutual_authentication: recvBuffer: "), recvBuffer, recvBufferLength);
#endif
...
#ifdef DEBUG
	OPGP_log_Hex(_T("mutual_authentication: key_diversification_data: "), key_diversification_data, 10);
	OPGP_log_Hex(_T("mutual_authentication: key_information_data: "), key_information_data, 2);
#endif
...
#ifdef DEBUG
	OPGP_log_Hex(_T("mutual_authentication:CardCryptogram: "), cardCryptogram, 8);
#endif
